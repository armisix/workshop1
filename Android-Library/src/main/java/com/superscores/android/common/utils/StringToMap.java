...
public final class StringToMap {
	 ...
	 */
	public static final String MAIN_KEY = "#main#"; // default key, if item has
													// no key
	...
	/**
	 * Convert array of characters (string) to map. The individual map items
	 * separated by iSep, key/value in each item is separated by keySep. Escape
	 * character is used to specify iSep or keySep in character array. E.g.:
	 * "key1:value1|key2:value2" will be converted to map of size 2 with
	 * content: ["key1":"value1", "key2":"value2"] One item can be without key,
	 * in this case it will be mapped under 'MAIN_KEY' key
	 *
	 * @param string
	 *            the character array representing string to be converted
	 * @param iSep
	 ...
	 * @param escape
	 *            escpae character
	 * @return Map representing map mapping of given character array or empty
	 *         map.
	...
	public static Map toMap(final char[] string, final char iSep,
		...
		for (int i = 0; i < string.length; i++) {
			...
			if ((string[i] == keySep) && (ikey2 < 0)) { // process this only if
														// we have not found key
														// yet
	...
	/**
	 * Convert array of characters (string) to map. The individual map items
	 * separated by defaultSeparator, key/value in each item is separated by
	 * defaultKeySeparator. Escape character used is defaultEscape
	 *
	 * @param chars
	 *            array of characters (String) to be converted to map
	...
	public static Map toMap(final char[] chars) {
		return toMap(chars, DEFAULT_ITEM_SEPARATOR, DEFAULT_KEY_SEPARATOR,
				DEFAULT_ESCAPE_CHARACTER);
	}

	/**
	 * Remove all occurences of (escape) character in the string. This helper
	 * method was inspired by http://www.rgagnon.com/javadetails/java-0030.html
	 *
	 * @param s
